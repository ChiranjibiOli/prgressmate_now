...
function getValue(PDO $pdo, string $query, array $params = [])
{
    try {
        $stmt = $pdo->prepare($query);
        $stmt->execute($params);
...
function updateLoginStreak($pdo, $user_id)
{
    ...
    if (!empty($user['last_login_date'])) {
        ...
        if ($last_login->format('Y-m-d') === $yesterday->format('Y-m-d')) {
            $new_streak = (int)$user['current_streak'] + 1;
        } else {
            $new_streak = 1;
        }
    } else {
...
function getStudentAchievements(PDO $pdo, int $student_id): array
{
    $stmt = $pdo->prepare("
        SELECT 
            a.id, a.title, a.description, a.icon, a.color, a.points,
            ua.earned_at
...
function getAchievementProgress(PDO $pdo, int $student_id): array
{
    // Fetch all active achievements not yet earned
    $stmt = $pdo->prepare("
        SELECT a.*
        FROM achievements a
    ...
    // Student stats used for criteria
    $completed_goals = (int)getStat($pdo, "
        SELECT COUNT(*) FROM student_goals
        WHERE student_id = ? AND status='completed' AND deleted_at IS NULL
    ", [$student_id]);

    $total_points = (int)getStat($pdo, "SELECT points FROM users WHERE id = ?", [$student_id]);
    $streak = (int)getStat($pdo, "SELECT current_streak FROM users WHERE id = ?", [$student_id]);

    $self_created_goals = (int)getStat($pdo, "
        SELECT COUNT(*) FROM student_goals
    ...
    foreach ($achievements as $a) {
        $target = (int)($a['criteria_value'] ?? 0);
        $type   = (string)($a['criteria_type'] ?? '');
        ...
        switch ($type) {
            ...
            case 'total_goals_created':
                $current = $self_created_goals;
                break;

            case 'completed_goals_in_category':
                // Your student_goals uses TEXT column "category" (name)
                if (!empty($a['category_id'])) {
                    $catName = getValue($pdo, "SELECT name FROM categories WHERE id = ?", [(int)$a['category_id']]);
                    if ($catName) {
                        $current = (int)getStat($pdo, "
                            SELECT COUNT(*) FROM student_goals
                            WHERE student_id = ?
                              AND category = ?
                              AND status='completed'
            ...
            default:
                // Unknown type => skip (so UI won’t break)
                continue 2;
        }

        $percentage = $target > 0 ? (int)min(100, round(($current / $target) * 100)) : 0;
