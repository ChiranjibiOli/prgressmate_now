    ...
    try {
        ...
        if ($action === 'add_achievement' || $action === 'edit_achievement') {
            ...
            if ($action === 'add_achievement') {
                ...
                $stmt->execute([
                    $title, $description, $points, $criteria_type, $criteria_value, $category_id,
                    $icon, $color, $is_active, $image_path
                ]);
                ...
                ");
                $stmt->execute([
                    $title, $description, $points, $criteria_type, $criteria_value,
                    $category_id, $icon, $color, $is_active, $image_path, $id
                ]);
        ...
        // AWARD TO ALL ELIGIBLE STUDENTS
            ...
            $students = $pdo->query("
                SELECT id FROM users WHERE role = 'student' AND status = 'active' AND deleted_at IS NULL
            ...
            foreach ($students as $student_id) {
                ...
                switch ($ach['criteria_type']) {
                    case 'total_completed_goals':
                        $count = getStat($pdo, "SELECT COUNT(*) FROM student_goals WHERE student_id = ? AND status = 'completed'", [$student_id]);
                    ...
                    case 'completed_goals_in_category':
                        if ($ach['category_id']) {
                            ...
                            if ($cat_name) {
                                $count = getStat($pdo, "SELECT COUNT(*) FROM student_goals WHERE student_id = ? AND category = ? AND status = 'completed'", [$student_id, $cat_name]);
                                $meets_criteria = ($count >= $ach['criteria_value']);
                            }
                        }
                        break;
                    ...
                    case 'total_goals_created':
                        $count = getStat($pdo, "SELECT COUNT(*) FROM student_goals WHERE student_id = ? AND is_self_created = 1", [$student_id]);
                        $meets_criteria = ($count >= $ach['criteria_value']);
                        break;
                }
